...
export class PermissionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!auth) throw new ForbiddenException('Authentication context is required.');
    const result = this.engine.evaluate(auth, { allPermissions: permissions, tenantId: request.context?.tenantId, branchId: request.context?.branchId });
    if (!result.allowed) throw new ForbiddenException(result.reason ?? 'Forbidden');
    return true;
  }
}
